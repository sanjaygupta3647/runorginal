		...
		{	nm=mesg; 
			...
			if(regBlank.test(val))
			{
				...
				else if(test.indexOf('isName')!=-1)
				{
					result = trim(val);
					if(result.length<3){
						errors += '- '+nm+' must contain at least 3 characters.\n'; 
						val_new.style.backgroundColor = '#e2e2e2';
					} 
				...
				else if (test.indexOf('isPincode')!=-1) 
				 {    
				...
				{   alert(val);
					if (isNaN(val))
					{
						errors+='- '+nm+' must be in year format like YYYY.\n';
						val_new.style.backgroundColor = '#e2e2e2';		
					}
			    }
				...
				else if (test.indexOf('isVldimage')!=-1) 
				{ 
					var new1 = val.toLowerCase()
					if ((new1.indexOf(".jpg") == -1 && new1.indexOf(".png") == -1))
					{
						errors+='- '+nm+' Only upload .jpg or .png.\n';
						val_new.style.backgroundColor = '#e2e2e2';		
					}
			    }
