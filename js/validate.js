var minYear=1900;var maxYear=2010;function isInteger(s){var i;for(i=0;i<s.length;i++){var c=s.charAt(i);if(((c<"0")||(c>"9")))return false;}
return true;}
...
if(i==2){this[i]=29}}
return this}
function isDate1(dtStr,dtCh){var dtCh=(dtCh==null)?"-":dtCh;var error='';var daysInMonth=DaysArray(12)
...
day=parseInt(strDay)
year=parseInt(strYr)
if(pos1==-1||pos2==-1){error+="- The date format should be : dd"+dtCh+"mm"+dtCh+"yyyy.\n";return error}
if(strMonth.length<1||month<1||month>12){error+="- Please enter a valid month\n";return error}
if(strDay.length<1||day<1||day>31||(month==2&&day>daysInFebruary(year))||day>daysInMonth[month]){error+="- Please enter a valid day.";return error}
if(strYear.length!=4||year==0||year<minYear||year>maxYear){error+="- Please enter a valid 4 digit year.\n";return error}
if(dtStr.indexOf(dtCh,pos2+1)!=-1||isInteger(stripCharsInBag(dtStr,dtCh))==false){error+="- Please enter a valid date.\n";return error}
return error;}
function isValidMobile1(mobileNumber){var error='';var mN='';var validChars='0123456789';for(var i=0;i<mobileNumber.length;i++){if(validChars.indexOf(mobileNumber.substr(i,1))>-1){mN+=mobileNumber.substr(i,1)}}
if(mN.length>0){if(mN.substr(0,1)=='0'){mN='44'+mN.substr(1)}
else if(mN.substr(0,3)=='440'){mN='44'+mN.substr(3)}}
else{error+="- Please specify a valid mobile number\n";return error}
if(mN.length<9){if(mN.length>7){if(!confirm('Please confirm that the mobile number you have\nentered is correct and in international format:.\n\n                      '+mN+'\n\nIf it is correct click \'OK\', otherwise click \'Cancel\'.')){error+='Sorry but the mobile number you have supplied is too short to be valid.\n'
return error}}
...
return true;}
function formvalid1()
{var i,p,q,nm,test,num,min,max,errors='',args=formvalid.arguments;j=0;var regEmail=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;var regBlank=/[^\s]/;var regAlphaNum=/^([a-zA-Z0-9_]+)$/;var regDate=/^([0-9_]+-[0-9][0-9]+-[0-9][0-9]+)$/;var formtype=document.forms[1].elements;var checked='';for(i=0;i<(formtype.length);i++)
{mesg=formtype[i].title;test=formtype[i].className;val=formtype[i];val_new=formtype[i];val_new.style.backgroundColor='';if(val)
{nm=mesg;val=val.value;if(regBlank.test(val))
{if(test.indexOf('isEqual')!=-1)
{equal_obj_val=test.substring(8,test.indexOf(":"));alert(equal_obj_val);mesg_string=test.substring((test.indexOf(":")+1));if(val!=formtype[equal_obj_val].value)
{errors+='- '+nm+' must be same to '+mesg_string+'.\n';formtype[equal_obj_val].style.backgroundColor='#e2e2e2';val_new.style.backgroundColor='#e2e2e2';}}
...
{return false;}}}
function checkall(objForm){len=objForm.elements.length;var i=0;for(i=0;i<len;i++){if(objForm.elements[i].type=='checkbox'){objForm.elements[i].checked=objForm.check_all.checked;if(objForm.check_all.checked==true)
{if(i==1)document.adminForm.eid.value=objForm.elements[i].value;}
else
{document.adminForm.eid.value='';}}}}
function checkall1(objForm){len=objForm.elements.length;var i=0;var j=1;for(i=0;i<len;i++){if(objForm.elements[i].type=='checkbox'){document.getElementById('cid'+j).checked=objForm.check_all.checked;if(objForm.check_all.checked==true)
{if(i==1)document.adminForm.eid.value=objForm.elements[i].value;}
else
{document.adminForm.eid.value='';}}
j++;}}
...
{found=false;for(i=0;i<fObj.length;i++)
{if(fObj[i].type=="checkbox"&&fObj[i].checked)
{found=true;break}}
return found;}
function editcheckBox(page,name){var val=document.adminForm.eid.value;if(!document.adminForm.eid.value){alert('Please select a '+name+' from the list to edit');return false;}else{if(!document.adminForm.option.value){gopg(page+'?eid='+val);}else{gopg(page+'eid='+val);}}}
function deletecheckBox(page,name){var fObj=document.adminForm;var val=getchecked();if(is_any_check_box_checked(fObj)==false)
{alert('Please select a '+name+' from the list to delete');return false;}else{gopg(page+'?Delete=yes&cids='+val);}}
function unpublishcheckBox(page,name){var fObj=document.adminForm;var val=getchecked();if(is_any_check_box_checked(fObj)==false)
{alert('Please select a '+name+' from the list to unpublish');return false;}else{gopg(page+'?Deactivate=yes&cids='+val);}}
function unfeaturedcheckBox(page,name){var fObj=document.adminForm;var val=getchecked();if(is_any_check_box_checked(fObj)==false)
{alert('Please select a '+name+' from the list to Unfeatured');return false;}else{gopg(page+'?Fno=yes&cids='+val);}}
function featuredcheckBox(page,name){var fObj=document.adminForm;var val=getchecked();if(is_any_check_box_checked(fObj)==false)
{alert('Please select a '+name+' from the list to Featured');return false;}else{gopg(page+'?Fyes=yes&cids='+val);}}
function hotNocheckBox(page,name){var fObj=document.adminForm;var val=getchecked();if(is_any_check_box_checked(fObj)==false)
{alert('Please select a '+name+' from the list to Hot No');return false;}else{gopg(page+'?Hot_No=yes&cids='+val);}}
function hotYescheckBox(page,name){var fObj=document.adminForm;var val=getchecked();if(is_any_check_box_checked(fObj)==false)
{alert('Please select a '+name+' from the list to Hot Yes');return false;}else{gopg(page+'?Hot_Yes=yes&cids='+val);}}
function publishcheckBox(page,name){var fObj=document.adminForm;var val=getchecked();if(is_any_check_box_checked(fObj)==false)
{alert('Please select a '+name+' from the list to publish');return false;}else{gopg(page+'?Activate=yes&cids='+val);}}
...
{alert("Enter Old Password !");oldpassword_skip.focus();return false;}
if(password.value=="")
{alert("Enter Password !");password.focus();return false;}
if(confirmpassword_skip.value=="")
{alert("Enter Confirm Password !");confirmpassword_skip.focus();return false;}
if(password.value!=confirmpassword_skip.value)
{alert("New Password doesn't match confirm password !");confirmpassword_skip.focus();return false;}
return true;}}
function getWordCount(val){val=val.toString()
...
function getchecked()
{var arr='';if(document.forms['adminForm'].elements['chk[]']){if(document.forms['adminForm'].elements['chk[]'].length)
...
return arr;}}
function formSubmit1(task){document.adminForm.task.value=task;document.adminForm.submit();return true;}
function formSubmit(task)
{document.adminForm.task.value=task;var frmObj=document.adminForm;if(formvalid(frmObj))
{frmObj.submit();return true;}
else
{return false;}}
function publish(id,val)
{document.getElementById('cid'+id).checked=true;var cids=getchecked();gopg('?'+val+'=yes&cids='+cids);}
function hot(id,val)
{document.getElementById('cid'+id).checked=true;var cids=getchecked();gopg('?'+val+'=yes&cids='+cids);}
function trcolor(id,val)
{if(val){id.className='trcolor-hover'}else{id.className='trcolor'}}
function submit_once(but,frmObj)
{if(formvalid(frmObj))
{frmObj.submit();but.disabled=true;return true;}
else
{return false;}}
...
{if(this.className=='selectBoxTime'){this.className='selectBoxTimeOver';this.getElementsByTagName('IMG')[0].src=pathToImages+'_down_time_over.gif';}else if(this.className=='selectBoxTimeOver'){this.className='selectBoxTime';this.getElementsByTagName('IMG')[0].src=pathToImages+'_down_time.gif';}
if(this.className=='selectBox'){this.className='selectBoxOver';this.getElementsByTagName('IMG')[0].src=pathToImages+'_down_over.gif';}else if(this.className=='selectBoxOver'){this.className='selectBox';this.getElementsByTagName('IMG')[0].src=pathToImages+'_down.gif';}}
function highlightArrow()
{if(this.src.indexOf('over')>=0){if(this.src.indexOf('left')>=0)this.src=pathToImages+'_left.gif';if(this.src.indexOf('right')>=0)this.src=pathToImages+'_right.gif';}else{if(this.src.indexOf('left')>=0)this.src=pathToImages+'_left_over.gif';if(this.src.indexOf('right')>=0)this.src=pathToImages+'_right_over.gif';}}
function highlightClose()
{if(this.src.indexOf('over')>=0){this.src=pathToImages+'_close.gif';}else{this.src=pathToImages+'_close_over.gif';}}
...
{var topBar=document.createElement('DIV');topBar.className='topBar';topBar.id='topBar';calendarDiv.appendChild(topBar);var leftDiv=document.createElement('DIV');leftDiv.style.marginRight='1px';var img=document.createElement('IMG');img.src=pathToImages+'_left.gif';img.onmouseover=highlightArrow;img.onclick=switchMonth;img.onmouseout=highlightArrow;leftDiv.appendChild(img);topBar.appendChild(leftDiv);if(Opera)leftDiv.style.width='16px';var rightDiv=document.createElement('DIV');rightDiv.style.marginRight='1px';var img=document.createElement('IMG');img.src=pathToImages+'_right.gif';img.onclick=switchMonth;img.onmouseover=highlightArrow;img.onmouseout=highlightArrow;rightDiv.appendChild(img);if(Opera)rightDiv.style.width='16px';topBar.appendChild(rightDiv);var monthDiv=document.createElement('DIV');monthDiv.id='monthSelect';monthDiv.onmouseover=highlightSelect;monthDiv.onmouseout=highlightSelect;monthDiv.onclick=showMonthDropDown;var span=document.createElement('SPAN');span.innerHTML=monthArray[currentMonth];span.id='calendar_month_txt';monthDiv.appendChild(span);var img=document.createElement('IMG');img.src=pathToImages+'_down.gif';img.style.position='absolute';img.style.right='0px';monthDiv.appendChild(img);monthDiv.className='selectBox';if(Opera){img.style.cssText='float:right;position:relative';img.style.position='relative';img.style.styleFloat='right';}
topBar.appendChild(monthDiv);var monthPicker=createMonthDiv();monthPicker.style.left='37px';monthPicker.style.top=monthDiv.offsetTop+monthDiv.offsetHeight+1+'px';monthPicker.style.width='60px';monthPicker.id='monthDropDown';calendarDiv.appendChild(monthPicker);var yearDiv=document.createElement('DIV');yearDiv.onmouseover=highlightSelect;yearDiv.onmouseout=highlightSelect;yearDiv.onclick=showYearDropDown;var span=document.createElement('SPAN');span.innerHTML=currentYear;span.id='calendar_year_txt';yearDiv.appendChild(span);topBar.appendChild(yearDiv);var img=document.createElement('IMG');img.src=pathToImages+'_down.gif';yearDiv.appendChild(img);yearDiv.className='selectBox';if(Opera){yearDiv.style.width='50px';img.style.cssText='float:right';img.style.position='relative';img.style.styleFloat='right';}
var yearPicker=createYearDiv();yearPicker.style.left='113px';yearPicker.style.top=monthDiv.offsetTop+monthDiv.offsetHeight+1+'px';yearPicker.style.width='35px';yearPicker.id='yearDropDown';calendarDiv.appendChild(yearPicker);var img=document.createElement('IMG');img.src=pathToImages+'_close.gif';img.style.styleFloat='right';img.onmouseover=highlightClose;img.onmouseout=highlightClose;img.onclick=closeCalendar;topBar.appendChild(img);if(!document.all){img.style.position='absolute';img.style.right='2px';}}
