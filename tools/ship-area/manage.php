...
<?php 
	...
	if($cms->is_post_back()){
		if($arr_ids) {
			...
			switch ($_POST['action']){
				case "delete":
					$cms->db_query("delete from #_shipping_area where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Delete', 'e');
					break;
				case "Inactive":
					$cms->db_query("update #_shipping_area set status = 'Inactive' where pid in ($str_adm_ids)");
					$adm->sessset(count($arr_ids).' Item(s) Inactive', 'e');
					break;
				case "Active":
					$cms->db_query("update #_shipping_area set status = 'Active' where pid in ($str_adm_ids)");
	...
	$columns = "select * ";
	$sql = " from #_shipping_area where 1 ";
